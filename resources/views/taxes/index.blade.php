                        ...
                        <div class="float-right d-none d-md-block">
                            <button class="btn_round_add btn-primary arrow-none waves-effect waves-light">
                                <i class="mdi mdi-plus mr-2"></i> <a style="color: #fff"
                                                                     href="{{ route('tax_add') }}">Ajouter une taxe</a>
                            </button>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="text-muted mb-4">
                                Liste de toutes les taxes appliquées aux transactions.
                            </p>
                            ...
                            <table id="datatable-buttons"
                                ...
                                @forelse ($taxes as $c)
                                    <tr>
                                        <td> {{ $c['code'] }}</td>
                                        <td> {{ $c['libelle'] }}</td>
                                        <td> {{ $c['type'] === 'pourcentage' ? 'Pourcentage' : 'Fixe' }}</td>
                                        <td> {{ $c['valeur'] }}{{ $c['type'] === 'pourcentage' ? ' %' : '' }}</td>
                                        <td> {{ $c['type'] === 'pourcentage' ? ($c['assiette'] ?? '—') : '—' }}</td>
                                        <td> {{ $c['zone']['name'] ?? 'Toutes zones' }}</td>
                                        <td>
                                            @if(!empty($c['statut']))
                            ...
                            <div class="modal fade" id="my_modal" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog" role="dialog">
                                    <form action="{{route('tax_delete')}}" method="post">
                                        {{ csrf_field() }}
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title" style="color: red">Suppression de la taxe</h4>
                                            </div>
                                            <div class="modal-body">
                                                Voulez-vous supprimer la taxe : <b id="ctryDel" style="color: red"></b>
                                                ?
